...
class Carousel(Widget):
    ...
    def _render_slides(self):
        ...
        js = (
            ...
            '  var w=0;'
            '  if(vp){'
            '    if(vp.getBoundingClientRect)w=Math.floor(vp.getBoundingClientRect().width||0);'
            '    if(!w)w=Math.floor(vp.clientWidth||vp.offsetWidth||0);'
            '  }'
            '  if(!w&&vp&&vp.parentElement){'
            '    if(vp.parentElement.getBoundingClientRect)w=Math.floor(vp.parentElement.getBoundingClientRect().width||0);'
            '    if(!w)w=Math.floor(vp.parentElement.clientWidth||vp.parentElement.offsetWidth||0);'
            '  }'
            '  return w;'
            '}'

            'function _singleThreshold(){'
            '  return Math.max(320,Math.min(bp,dvis*180+gap*Math.max(dvis-1,0)));'
            '}'

            'function _vis(){'
            '  var mobileByViewport=!!(window.matchMedia&&window.matchMedia("(max-width:"+bp+"px)").matches);'
            '  var mobileByWidth=!!(_hostW()&&_hostW()<=_singleThreshold());'
            '  isMobile=mobileByViewport||mobileByWidth;'
            ...
            '  var _ro=new ResizeObserver(function(){_syncMode();});'
            '  _ro.observe(vp);'
            '  if(vp.parentElement)_ro.observe(vp.parentElement);'
            '}'
            '})();'
        )
