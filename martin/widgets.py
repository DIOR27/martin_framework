...
class Table(Widget):
    ...
    def render(self):
        ...
        for i, (plain, html_cells) in enumerate(rows_data):
            stripe = (
                f"background:var(--surface-2,rgba(0,0,0,0.03))"
...
class Modal(Widget):
    ...
    def render(self):
        ...
        backdrop_click = (
            f" onclick=\"if(event.target===this)closeModal('{mid}')\""
...
class Map(Widget):
    ...
    def render(self):
        ...
        init_js = (
            ...
            "  maxZoom:19,"
            "  crossOrigin:true"
            "}).addTo(_map);" + icon_fn + click_fn + "var _markers=" + J(mkrs) + ";"
            "var _lls=[];"
            ...
            "  if(_ph)_mk.bindPopup(_ph);" + click_bind + "  _lls.push([m.lat,m.lon]);"
...
class CookieBanner(Widget):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.show_customize = (
            show_customize if show_customize is not None else bool(self.categories)
        )
    ...
    def render(self):
        ...
        if cats:
            ...
            for cat in cats:
                ...
                rows += (
                    ...
                    'style="width:42px;height:24px;border-radius:12px;cursor:'
                    + cursor
                    + ";"
        ...
        if is_modal:
            ...
            edge = (
                "bottom:0;border-top:1px solid var(--border);"
                if pos != "top"
                else "top:0;border-bottom:1px solid var(--border);"
            )
            outer_style = (
        ...
        content_html = (
            ...
            + layout
            + ";gap:"
            + ("16px" if is_modal else "32px")
            + ";"
            "flex-wrap:wrap;align-items:"
            + ("flex-start" if is_modal else "center")
            + ';">'
        ...
        js = (
            ...
            "  save(c);hide();" + (on_accept_js + "(c);" if on_accept_js else "") + "};"
            ...
            "  save(c);hide();" + (on_reject_js + "(c);" if on_reject_js else "") + "};"
            # Save custom
